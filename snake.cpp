...
public:
    ...
    void Generate_Loss()
    {
        if (streak == 6)
        {
            loss.setLossX((rand() % witdh) + 60);
            loss.setLossY((rand() % height) + 12);
            // if (loss.getLossX() == food.getFoodX() && loss.getLossY() == food.getFoodY())
            // {
            //     return Generate_Loss();
            // }
            for (int i = 0; i < Medusa.getSnakeLength(); i++)
            {
                if (loss.getLossX() == Medusa.getSnakeX(i) && loss.getLossY() == Medusa.getSnakeY(i))
                {
                    Generate_Loss();
                    break;
                }
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < witdh; j++)
            {
                ...
                else if ((i + 12) == food.getFoodY() && (j + 60) == food.getFoodX())
                {
                    ...
                    if(timeLoss&&loss.getLossY()==(i+12)&&loss.getLossX()==(j+60)){
